...
class Device:
    def __init__(self):
        ...
        try:
            self.install_mode = re.search(
                r'Installation mode is\s+(\S+)',
...
def main():
    ...
    try:
        ...
        log_info('Flash size/free (MB)...........: ' + str(round(device.flash_size / 1024 / 1024)) + '/' +
                 str(round(device.get_flash_size_free() / 1024 / 1024)))
        log_info('USB present....................: ' + str(device.get_usp_present()))
        ...
        switch_to_install_mode = True if not device.install_mode and \
                                         target_software.install_mode and \
                                         model.install_mode else False

        log_info('Switch to install mode required: ' + str(switch_to_install_mode))
        ...
        if update_status:
            ...
            if not check_file_exists(target_software.image):
                file_transfer(http_image, target_software.image, device.get_usp_present())
            ...
            if check_file_exists(target_software.image):
                log_info('Performing activate image ' + target_software.image + ' - device will reboot')
...
def switch_to_install(image):
    log_info('Performing switch to install mode - device will reboot')
    sleep_time_min = 20
    configurep([
        'no boot system',
    ...
    executep('write memory')
    executep('write erase')
    log_info('Wait ' + str(sleep_time_min) + ' minutes to get the change to install mode done...')
...
def file_transfer(http_server, file, usb=False):
    ...
    if usb and check_file_exists(file, 'usb0:/'):
        ...
        log_info('Depending on file size and speed this can take some time (up to 1 hour or more)')
