...
models = {
    'C9300-24P': {
        'software_image': 'CAT9K',
    },
    'C9500-24Q': {
        'software_image': 'CAT9K',
    },
    'ASR1001-HX': {
        'software_image': 'ASR1000',
    },
    'C1117-4PMLTEEANE': {
        'software_image': 'C1100',
    }
}
...
def main():
    try:
        ...
        software_image = software_images[models[model]['software_image']]
        software_version = software_images[models[model]['software_version']]
        software_md5_checksum = software_images[models[model]['software_md5_checksum']]
...
def configure_replace(file, file_system='flash:/'):
    config_command = f'configure replace {file_system}{file} force'
...
def configure_merge(file, file_system='flash:/'):
    ...
    config_command = f'copy {file_system}{file} running-config'
    config_repl = executep(config_command)
    time.sleep(120)
...
def deploy_eem_cleanup_script():
    ...
    eem_commands = [
        f'event manager applet cleanup',
        f'event none maxrun 600',
        f'action 1.0 cli command "enable"',
        f'action 2.0 cli command "{install_command}" pattern "\[y\/n\]"'
        f'action 2.1 cli command "y" pattern "proceed"',
        f'action 2.2 cli command "y"'
    ]
...
def deploy_eem_upgrade_script(image):
    install_command = f'install add file flash://{image} activate commit'
    eem_commands = [
        f'event manager applet upgrade',
        f'event none maxrun 600',
        f'action 1.0 cli command "enable"',
        f'action 2.0 cli command "{install_command}" pattern "\[y\/n\/q\]"' 
        f'action 2.1 cli command "n" pattern "proceed"',
        f'action 2.2 cli command "y"'
    ]
    results = configurep(eem_commands)
